...
function resolveJudgeModel(config?: JudgeConfig): { model: string; provider: AIProvider; apiKey: string } {
...
async function callJudge(prompt: string, config?: JudgeConfig): Promise<{ score: number; pass: boolean; reason: string; tokensUsed: number; provider: string; model: string }> {
  ...
  if (provider !== "anthropic") {
    const compat = createOpenAICompatibleConfig(provider as OpenAICompatProvider, apiKey);
